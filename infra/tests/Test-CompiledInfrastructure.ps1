...
if ($Mode -eq 'dev') {
    ...
    Assert-True ($keyVaultAssignments.Count -eq 4 -and @($keyVaultAssignments | Where-Object { $_.scope -notmatch 'Microsoft.KeyVault/vaults' }).Count -eq 0) 'API, web, and migration identities must receive Key Vault Secrets User only at the vault scope'
    ...
    Assert-True ($webAssignments.Count -eq 2 -and @($webAssignments | Where-Object { $_.name -notmatch 'guid\(' }).Count -eq 0) 'web role assignment names must be deterministic GUID expressions'
