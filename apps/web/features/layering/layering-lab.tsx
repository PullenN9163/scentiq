...
export function LayeringLab({ owned, suggestions, initialA }: { owned: FragranceSummary[]; suggestions: LayeringSuggestion[]; initialA?: string }) {
  ...
  const suggestion = useMemo(() => suggestions.find((item) => item.mode === mode && new Set([item.first.id, item.second.id]).has(firstId) && new Set([item.first.id, item.second.id]).has(secondId) && firstId !== secondId), [suggestions, mode, firstId, secondId]);
