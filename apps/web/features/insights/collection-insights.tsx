...
import { collection, fragrances, getDemoFragranceById, getDemoInsights } from "@/lib/demo";
...
export function CollectionInsights() {
  ...
  const metrics = [
    ...
    ["Most worn", getDemoFragranceById(insights.mostWornId)!.name], ["Least worn", getDemoFragranceById(insights.leastWornId)!.name], ["Highest rated", getDemoFragranceById(insights.highestRatedId)!.name], ["Best cost / wear", getDemoFragranceById(insights.bestCostPerWearId)!.name],
  ];
  const ownershipCounts = ["Bottle", "Decant", "Sample"].map((type) => ({ type, count: collection.filter((item) => item.ownershipType === type).length }));
  const noteLayers = ["Top", "Middle", "Base"].map((layer) => ({ layer, count: fragrances.slice(0, collection.length).flatMap((item) => item.notes).filter((note) => note.layer === layer).length }));
  return (
    <section className="page">
      ...
      <div className="grid grid-2 insight-notes"><Card><CardContent><p className="eyebrow">Ownership mix</p><h2 className="serif">Formats on the shelf</h2>{ownershipCounts.map((item) => <p key={item.type}><strong>{item.type}</strong> · {item.count}</p>)}</CardContent></Card><Card><CardContent><p className="eyebrow">Note architecture</p><h2 className="serif">Top, middle, and base</h2>{noteLayers.map((item) => <p key={item.layer}><strong>{item.layer}</strong> · {item.count} represented notes</p>)}</CardContent></Card></div>
