...
export function AgentExperience() {
  ...
  return <section className="page agent-page"><PageHeader eyebrow="Deterministic demo advisor" title="Ask ScentIQ" description="Fast answers grounded in the same demo collection—without pretending to understand arbitrary prompts." /><div className="agent-shell"><div className="conversation" aria-live="polite">{messages.length === 0 && <div className="agent-welcome"><span><Sparkles /></span><h2 className="serif">What would you like to decide?</h2><p>Choose a supported intent to see how a future fragrance advisor could use your context.</p></div>}{messages.map((message, index) => <div className="exchange" key={`${message.prompt}-${index}`}><p className="user-bubble">{message.prompt}</p><div className="agent-bubble"><Badge>Demo response</Badge><p>{message.answer.text}</p><Link href={message.answer.href}>{message.answer.link} <ArrowRight size={15} /></Link></div></div>)}</div><div className="quick-prompts">{prompts.map((prompt) => <button key={prompt} onClick={() => ask(prompt)}>{prompt}</button>)}</div><form className="agent-input" onSubmit={submit}><label className="sr-only" htmlFor="agent-question">Ask ScentIQ</label><input id="agent-question" value={input} onChange={(event) => setInput(event.target.value)} placeholder="Try a supported question…" /><Button type="submit"><Send size={16} />Send</Button></form></div></section>;
}
